...
function replayBufferedMessages(socket) {
...
function serveVideo(sessionId, reply) {
...
export async function start(opts = {}) {
    ...
    server.get('/client', { websocket: true }, (socket, _req) => {
        log.info(`client connected (replaying ${messageBuffer.length} buffered message(s))`);
        ...
        socket.on('close', () => {
            ...
            if (clients.size === 0 && workerSocket?.readyState === WebSocket.OPEN) {
                workerSocket.send(JSON.stringify({ scope: 'clientDisconnected', data: {} }));
            }
        });
        if (workerSocket?.readyState === WebSocket.OPEN) {
            workerSocket.send(JSON.stringify({ scope: 'clientConnected', data: {} }));
        }
    });
    server.get('/worker', { websocket: true }, (socket, _req) => {
        ...
        socket.on('message', (message) => {
            ...
            log.debug(`received ${message.length} byte message from worker to ${clients.size} client${clients.size > 1 ? 's' : ''}`);
            try {
    ...
    server.get('/api/video/:sessionId', {
